...
/* lijiangshuo add for backlight 20140530 start */
#define WLED_NAME "wled-backlight"
#define LOOP_MAX_SIZE 33
/* lijiangshuo add for backlight 20140530 end */
/**
	...
	if (led->cdev.brightness) {
		...
		if (led->mpp_cfg->pwm_mode != MANUAL_MODE)
			...
			rc = qpnp_led_masked_write(led,
					...
					LED_MPP_SINK_MASK, LED_MPP_CURRENT_MIN);
				//	LED_MPP_SINK_MASK, val);     //zhoufan changed for blk 20140605
			if (rc) {
				dev_err(&led->spmi_dev->dev,
...
/* lijiangshuo add for backlight 20140530 start */
		if(!strncmp(WLED_NAME,led->cdev.name,sizeof(WLED_NAME))) {
			//printk("zhoufan\n");
			rc= qpnp_led_masked_write(led,
			...
			loop = (255 - led->cdev.brightness) * 31 / 255 + 1;
			//printk("zhoufan %s loop=%d led->loop=%d\n", __func__, loop, led->loop);
			if (loop == led->loop) {
				return 0;
			}
			//printk("zhoufan %s loop=%d led->loop=%d\n", __func__, loop, led->loop);
...
/* lijiangshuo add for backlight 20140530 end */
		...
		led->loop = 0; // lijiangshuo add for backlight 20140530
	}
...
static int __devinit qpnp_led_set_max_brightness(struct qpnp_led_data *led)
{
	...
	case QPNP_ID_LED_MPP:
		if (led->mpp_cfg->pwm_mode == MANUAL_MODE)
		//	led->cdev.max_brightness = led->max_current;
			led->cdev.max_brightness = MPP_MAX_LEVEL;   //zhoufan add for BKL 20140605
		else
				...
				struct device_node *node)
{
	...
	if (!rc)
		...
		return rc;
    led->loop = 0;         //zhoufan add for BKL  20140603
...
static int __devinit qpnp_leds_probe(struct spmi_device *spmi)
{
	...
	for_each_child_of_node(node, temp) {
		...
		mutex_init(&led->lock);
		spin_lock_init(&led->spinlock);      //zhoufan add for backlight 20140603
